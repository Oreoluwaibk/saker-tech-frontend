...
import AddPurpose from '@/components/modals/visitorbook/AddPurpose';
import AddVisitorBook from '@/components/modals/visitorbook/AddVisitorBook';
...
const index = () => {
  ...
  const [ openAddPurpose, setOpenAddPurpose ] = useState(false);
  const [ openAddVisitors, setOpenAddVisitors ] = useState(false);
  ...
  return (
  <DashboardContainer name='Visitor Book' activeKey="3">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Visitor Book</p>
        <p className='text-base text-[#616161] font-light'>Track and manage all visitor Book.</p>
      </div>}
      extra={<div className='flex gap-4'>
        <Button type="default" className='!h-[54px] !w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAddPurpose(true)}>Add Purpose</Button>
        <Button type="primary" className='!h-[54px] !w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAddVisitors(true)}>Add Visitor</Button>
      </div>}
    >
    ...
    {openAddVisitors && <AddVisitorBook open={openAddVisitors} onCancel={() => setOpenAddVisitors(false)} />}
    {openAddPurpose && <AddPurpose open={openAddPurpose} onCancel={() => setOpenAddPurpose(false)} />}
  </DashboardContainer>
  )
}
