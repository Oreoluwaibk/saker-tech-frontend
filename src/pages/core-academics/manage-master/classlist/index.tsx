...
const index = () => {
  ...
  return (
    <>
      <TableCards 
        name='Class List'
        description='Manage and Track Classes'
        title='Class List'
        activeKey='4.2'
        column={column}
        dataSource={courses}
        extra={<div className='flex gap-4'>
          <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddClasses(true)}>Add Class</Button>
        </div>}
        showFilter
      />

      {addClasses && <AddClasses onCancel={() => setAddClasses(false)} open={addClasses} />}
    </>
