...
import AddReason from '@/components/modals/AddReason';
import AddBoardList from '@/components/modals/AddBoard';

const index = () => {
  const [ select, setSelect ] = useState<any>(null);
  const [ deleteLoading, setDeleteLoading ] = useState<boolean>(false);
  const [ courses, setCourses ] = useState([]);
  const [ addBoardList, setAddBoardList ] = useState(false)
  ...
  return (
    <>
      <TableCards 
        name='Board List'
        description='Manage and Track Board List'
        title='Board List'
        activeKey='4.4'
        column={column}
        dataSource={courses}
        extra={<div className='flex gap-4'>
          <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddBoardList(true)}>Add Board</Button>
        </div>}
        showFilter
      />

      {addBoardList && <AddBoardList onCancel={() => setAddBoardList(false)} open={addBoardList} />}
    </>
