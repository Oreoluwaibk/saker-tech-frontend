...
const index = () => {
  ...
  return (
    <>
      <TableCards 
        name='Level List'
        description='Manage and Track Level'
        title='Level List'
        activeKey='4.9'
        column={column}
        dataSource={courses}
        extra={<div className='flex gap-4'>
          <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddLevel(true)}>Add Level</Button>
        </div>}
        showFilter
      />

      {addLevel && <AddLevel onCancel={() => setAddLevel(false)} open={addLevel} />}
    </>
