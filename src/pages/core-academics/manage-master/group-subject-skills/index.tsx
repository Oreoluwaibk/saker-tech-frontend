...
import AddGroupSubject from '@/components/modals/AddGroupSubject';

const index = () => {
  const [ select, setSelect ] = useState<any>(null);
  const [ deleteLoading, setDeleteLoading ] = useState<boolean>(false);
  const [ courses, setCourses ] = useState([]);
  const [ addGroupSubject, setAddGroupSubject ] = useState(false)
  ...
  return (
    <>
      <TableCards 
        ...
        description='Manage and Track Group Subjects'
        title='Group Subject List'
        activeKey='4.8'
        column={column}
        dataSource={courses}
        extra={<div className='flex gap-4'>
          <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddGroupSubject(true)}>Add Group Subject</Button>
        </div>}
        showFilter
      />

      {addGroupSubject && <AddGroupSubject onCancel={() => setAddGroupSubject(false)} open={addGroupSubject} />}
    </>
