...
const index = () => {
  ...
  return (
    <>
      <TableCards 
        name='Reason List'
        description='Manage and Track Subject Category'
        title='Reason List'
        activeKey='4.3'
        column={column}
        dataSource={courses}
        extra={<div className='flex gap-4'>
          <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddReason(true)}>Add Reason</Button>
        </div>}
        showFilter
      />

      {addReason && <AddReason onCancel={() => setAddReason(false)} open={addReason} />}
    </>
