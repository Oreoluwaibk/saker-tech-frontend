...
import AddCategoryList from '@/components/modals/AddCategoryList';

const index = () => {
  const [ select, setSelect ] = useState<any>(null);
  const [ deleteLoading, setDeleteLoading ] = useState<boolean>(false);
  const [ courses, setCourses ] = useState([]);
  const [ addCatgeory, setAddCategory ] = useState(false)
  ...
  return (
    <>
      <TableCards 
        name='Category List'
        description='Manage and Track Category'
        title='Category List'
        activeKey='4.9'
        column={column}
        dataSource={courses}
        extra={<div className='flex gap-4'>
          <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddCategory(true)}>Add Category</Button>
        </div>}
        showFilter
      />

      {addCatgeory && <AddCategoryList onCancel={() => setAddCategory(false)} open={addCatgeory} />}
    </>
