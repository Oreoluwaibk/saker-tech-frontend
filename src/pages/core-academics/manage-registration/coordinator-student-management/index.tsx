...
const index = () => {
  ...
  const column: ColumnsType = [
      ...
      {
          key: "2",
          title: "Coordinator Name(Login ID)",
          dataIndex: "price",
          render(value) {
          if(value){
  ...
  return (
    <>
    <TableCards 
      name='Coordinator/Student Mapping List'
      description='Manage and coordinate student.'
      title='Coordinator/Student Mapping List'
      activeKey='6.9'
      column={column}
      showFilter
      dataSource={courses}
      extra={<div className='flex gap-4'>
        <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddAcademicYear(true)}>Add Coordinator Mapping</Button>
      </div>}
    />
