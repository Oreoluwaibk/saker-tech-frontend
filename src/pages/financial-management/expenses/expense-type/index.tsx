...
const index = () => {
    ...
    const [ openAddExpenseType, setAddExpenseType ] = useState(false);
  ...
  return (
    <DashboardContainer name='Expense Type List' activeKey="19.2">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Expense Type List</p>
        <p className='text-base text-[#616161] font-light'>Manage and track list of expense types</p>
      </div>}
      extra={<div className='flex gap-4'>
        
        <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setAddExpenseType(true)}>Add Expense Type</Button>
      </div>}
    >
    ...
    {openAddExpenseType && <AddExpenseType open={openAddExpenseType} onCancel={() => setAddExpenseType(false)} />}
    </DashboardContainer>
  )
}
