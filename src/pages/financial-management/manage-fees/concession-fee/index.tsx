...
import AddConcessionFee from '@/components/modals/financialManagenent/manageFee/AddConcessionFee';
...
const index = () => {
    ...
    const [ openAddConcession, setOpenAddConcession ] = useState(false);
  ...
  return (
    <DashboardContainer name='Concession Fee List' activeKey="21.2">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Concession Fee List</p>
        <p className='text-base text-[#616161] font-light'>Manage and track list of concession fee</p>
      </div>}
      extra={<div className='flex gap-4'>
        <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAddConcession(true)}>Add Concession Fee</Button>
      </div>}
    >
    ...
    {openAddConcession && <AddConcessionFee open={openAddConcession} onCancel={() => setOpenAddConcession(false)} />}
    </DashboardContainer>
  )
}
