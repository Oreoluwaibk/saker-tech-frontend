...
import ApplyConcession from '@/components/modals/financialManagenent/manageFee/ApplyConcession';
import ApplyMasterFee from '@/components/modals/financialManagenent/manageFee/AddMasterFee';
...
const index = () => {
  ...
  return (
    <DashboardContainer name='Manage Fee List' activeKey="21.1">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Manage Fee List</p>
        <p className='text-base text-[#616161] font-light'>Manage and track list of master fee</p>
      </div>}
      extra={<div className='flex gap-4'>
        <Button type="default" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setApplyFee(true)}>Apply Concession Fee</Button>
        <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAddMaster(true)}>Add Master Fee</Button>
      </div>}
    >
    ...
    {applyFee && <ApplyConcession open={applyFee} onCancel={() => setApplyFee(false)} />}
    {openAddMaster && <ApplyMasterFee open={openAddMaster} onCancel={() => setOpenAddMaster(false)} />}
    </DashboardContainer>
  )
}
