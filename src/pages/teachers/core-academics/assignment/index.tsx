...
import CreateAssignment from '@/components/teachers/modal/CreateAssignment';
...
const index = () => {
  ...
  const [ selected, setSelected ] = useState("Draft List");
  ...
  const column: ColumnsType = [
    ...
    },
    {
      ...
      title: "Created On",
      dataIndex: "duration",
      hidden: selected !== "Draft List" && selected !== "Verified List"
    },
    {
      key: "6",
      title: "Published On",
  ...
  return (
  <TeacherDashboard name='Assignment' activeKey='1.5'>
    <Card
      variant="borderless"
      ...
      extra={selected === "Draft List" && <div className='flex gap-4'>
        <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold'  onClick={() => setOpenModal(true)}>Create Assignment</Button>
      </div>}
    >
      <div className='flex items-center gap-4'>
        <Selector 
          title='Draft List'
          selected={selected === "Draft List"}
          onclick={() => setSelected("Draft List")}
        />

        <Selector 
          title='Verified List'
          selected={selected === "Verified List"}
          onclick={() => setSelected("Verified List")}
        />

        <Selector 
          title='Published List'
          selected={selected === "Published List"}
          onclick={() => setSelected("Published List")}
        />
    ...
    {openModal && <CreateAssignment onCancel={() => setOpenModal(false)} open={openModal} />}
  </TeacherDashboard>
  )
}
