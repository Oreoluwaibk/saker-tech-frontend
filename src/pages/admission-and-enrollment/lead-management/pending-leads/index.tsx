...
import AddNewLeadsModal from '@/components/modals/leadMgt/AddNewLeadsModal';
import AssignLeadModal from '@/components/modals/leadMgt/AssignLeadModal';
...
const index = () => {
  ...
  return (
    <DashboardContainer name='Manage Pending Leads' activeKey="31.1">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Pending Leads</p>
        <p className='text-base text-[#616161] font-light'>Manage and track lists of pending leads</p>
      </div>}
      extra={<div className='flex gap-4'>
         <Button type="default" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAssign(true)}>Assign Leads</Button>
        <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAdd(true)}>Add New Leads</Button>
      </div>}
    >
    ...
    {openAssign && <AssignLeadModal open={openAssign} onCancel={() => setOpenAssign(false)} />}
    {openAdd && <AddNewLeadsModal open={openAdd} onCancel={() => setOpenAdd(false)} />}
