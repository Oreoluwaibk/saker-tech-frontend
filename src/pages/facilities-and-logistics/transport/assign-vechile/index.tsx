...
import AssignVechile from '@/components/modals/facilityAndLogistics/AssignVechile';
...
const index = () => {
    ...
    const [ openAssignVechile, setOpenAssignVechile ] = useState(false);
  ...
  return (
    <DashboardContainer name='Assigned Transport List' activeKey="15.3">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Assigned Transport List</p>
        <p className='text-base text-[#616161] font-light'>Manage and track vehicle</p>
      </div>}
      extra={<div className='flex gap-4'>
        
        <Button type="primary" className='!h-[54px] !w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAssignVechile(true)}>Assign Vechile</Button>
      </div>}
    >
    ...
    {openAssignVechile && <AssignVechile open={openAssignVechile} onCancel={() => setOpenAssignVechile(false)} />}
    </DashboardContainer>
  )
}
