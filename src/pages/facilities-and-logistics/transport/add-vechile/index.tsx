...
const index = () => {
    ...
    const [ openAddVechile, setOpenAddVechile ] = useState(false);
  ...
  return (
    <DashboardContainer name='Vehicle Transport List' activeKey="15.2">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Vehicle Transport List</p>
        <p className='text-base text-[#616161] font-light'>Manage and track lists of vehicle transport</p>
      </div>}
      extra={<div className='flex gap-4'>
        
        <Button type="primary" className='!h-[54px] !w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAddVechile(true)}>Add Vechile</Button>
      </div>}
    >
    ...
    {openAddVechile && <AddVechile open={openAddVechile} onCancel={() => setOpenAddVechile(false)} />}
    </DashboardContainer>
  )
}
