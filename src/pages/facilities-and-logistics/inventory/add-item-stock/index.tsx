...
import ItemIssue from '@/components/modals/facilityAndLogistics/inventory/ItemIssue';
import ItemStocked from '@/components/modals/facilityAndLogistics/inventory/ItemStocked';
...
const index = () => {
    ...
    const [ openItemStocked, setOpenItemStocked ] = useState(false);
  ...
  return (
    <DashboardContainer name='Item Stock List' activeKey="18.2">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Item Stock List</p>
        <p className='text-base text-[#616161] font-light'>Manage and track lists of item stocked</p>
      </div>}
      extra={<div className='flex gap-4'>
        
        <Button type="primary" className='!h-[54px] !w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenItemStocked(true)}>Add Item Stock</Button>
      </div>}
    >
    ...
    {openItemStocked && <ItemStocked open={openItemStocked} onCancel={() => setOpenItemStocked(false)} />}
    </DashboardContainer>
  )
}
