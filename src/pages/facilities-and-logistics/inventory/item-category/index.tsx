...
const index = () => {
  ...
  return (
    <DashboardContainer name='Item Category' activeKey="18.4">
    <Card
      variant="borderless"
      styles={{body: {backgroundColor: "#F6F6F6"}, header: {backgroundColor: "#f6f6f6"}}}
      title={<div className='pt-5 pb-3 px-4'>
        <p className='text-[30px] text-[#050505] font-semibold'>Item Category</p>
        <p className='text-base text-[#616161] font-light'>Manage and track items</p>
      </div>}
      extra={<div className='flex gap-4'>
        
        <Button type="primary" className='!h-[54px] !min-w-[160px] !text-[#3E444C] !rounded-[5px] !font-semibold' icon={<PlusOutlined />} onClick={() => setOpenAddItemStore(true)}>Add Item Category</Button>
      </div>}
    >
    ...
    {openAddItemStore && <ItemCategory open={openAddItemStore} onCancel={() => setOpenAddItemStore(false)} />}
    </DashboardContainer>
  )
}
